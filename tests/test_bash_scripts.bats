...
EOF
    ...
    # Create a mock generate_dashboard_refactored.sh
...
./scripts/generate_dashboard_refactored.sh || exit 1
...
@test "all scripts are executable" {
    ...
    [ -x "$PROJECT_DIR/scripts/generate_dashboard_refactored.sh" ]
}
...
@test "all scripts have valid bash syntax" {
    ...
    run bash -n "$PROJECT_DIR/scripts/generate_dashboard_refactored.sh"
    [ "$status" -eq 0 ]
}

# Tests for generate_dashboard_refactored.sh
@test "generate_dashboard_refactored.sh: generates dashboard successfully" {
    # Create a mock Elasticsearch response
    mkdir -p data
    cat > data/mock_response.json << 'EOF'
{
  "aggregations": {
    "events": {
      "buckets": [
        {
          "key": "pandc.vnext.recommendations.feed.feed_test.impression",
          "current": {"doc_count": 1000},
...
@test "generate_dashboard_refactored.sh: extracts cookie from local script" {
    unset ELASTIC_COOKIE
...
@test "generate_dashboard_refactored.sh: fails when no cookie available" {
    unset ELASTIC_COOKIE
    rm -f "$HOME/scripts/traffic_monitor.sh"
