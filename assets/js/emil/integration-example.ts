...
export async function completeWorkflowExample() {
  ...
  try {
    ...
    await metricsStore.processQueryResult(
      healthResult,
      'healthCheck',
      '1h'
    );
    ...
    if (oldEIDs.length > 0) {
      ...
      const batchResult = await registryService.batchUpdateStatus(
        eidIds,
        EIDStatus.OLD
      );
  ...
  const checkHealth = async () => {
    try {
      ...
      if (result.data && Array.isArray(result.data)) {
        const criticalEIDs = result.data.filter(
          (r: any) => r.health_status === 'CRITICAL'
        );
        
        if (criticalEIDs.length > 0) {
          console.warn(`ALERT: ${criticalEIDs.length} EIDs in critical state!`);
          criticalEIDs.forEach((eid: any) => {
...
export async function dashboardDataExample() {
  ...
  try {
    ...
    // Sort lists
    dashboard.topPerformers.sort((a, b) => 
      (b.latest_metrics?.health_score || 0) - (a.latest_metrics?.health_score || 0)
    );
    dashboard.needsAttention.sort((a, b) => 
      (a.latest_metrics?.health_score || 100) - (b.latest_metrics?.health_score || 100)
    );
