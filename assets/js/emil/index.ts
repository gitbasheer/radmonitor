...
registry: EIDRegistry;
  ...
  let selector: EIDSelector | undefined;
  if (options?.container) {
    selector = new EIDSelector({
      container: options.container,
      registry,
onSelect: (eids: string[]) => {
        console.log('Selected EIDs:', eids);
      },
      onSelectionChange: (eids: string[]) => {
 ...
 */
function generateMockEIDs(): EIDMetadata[] {
...
tags: [namespace, radset, radId, subaction].filter((tag): tag is string => tag !== undefined)
          });
        }

        idCounter++;
        if (idCounter > 10000) break;
      }
      if (idCounter > 10000) break;
    }
    if (idCounter > 10000) break;
