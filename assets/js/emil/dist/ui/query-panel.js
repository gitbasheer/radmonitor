...
export class QueryPanel {
    ...
    updateUI() {
        ...
        buttons.forEach(btn => {
            btn.disabled = !hasEIDs || this.isLoading;
        });
        // Update status
     ...
     */
    async executeQuery(action) {
        if (this.selectedEIDs.length === 0 || this.isLoading)
            return;
        this.isLoading = true;
        ...
        try {
            ...
            const timeWindow = document.getElementById('emil-time-window')?.value || '1h';
            const baselinePeriod = document.getElementById('emil-baseline-period')?.value || 'last-week';
            // Build intent
            const intent = {
                action: action,
     ...
     */
    buildParameters(action, timeWindow, baselinePeriod) {
        const params = {
...
//# sourceMappingURL=query-panel.js.map