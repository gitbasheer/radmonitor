...
export class EIDMetricsStore {
     ...
     */
    async processQueryResult(result, templateId, timeWindow) {
        ...
        try {
            ...
            const storePromises = metrics.map(metric => this.registryService.storeMetrics(metric.eid_id, {
                ...metric,
     ...
     */
    extractMetrics(data, templateId, timeWindow) {
        if (!Array.isArray(data)) {
            return [];
        }
        switch (templateId) {
            case 'healthCheck':
                return this.extractHealthCheckMetrics(data, timeWindow);
            case 'baselineComparison':
                return this.extractBaselineMetrics(data, timeWindow);
            case 'performanceMetrics':
                return this.extractPerformanceMetrics(data, timeWindow);
            default:
     ...
     */
    extractHealthCheckMetrics(results, timeWindow) {
     ...
     */
    extractBaselineMetrics(results, timeWindow) {
        return results.map(result => ({
            eid_id: result.eid,
     ...
     */
    extractPerformanceMetrics(results, timeWindow) {
        return results.map(result => ({
            eid_id: result.eid,
     ...
     */
    extractGenericMetrics(results, timeWindow) {
        return results.map(result => {
            const metric = {
            ...
            if ('event_count' in result)
                metric.event_count = result.event_count;
            if ('avg_latency' in result)
                metric.avg_latency = result.avg_latency;
            if ('p95_latency' in result)
                metric.p95_latency = result.p95_latency;
            if ('p99_latency' in result)
                metric.p99_latency = result.p99_latency;
            if ('error_rate' in result)
                metric.error_rate = result.error_rate;
            if ('success_rate' in result)
                metric.success_rate = result.success_rate;
            if ('health_status' in result)
                metric.health_status = result.health_status;
            if ('health_score' in result)
                metric.health_score = result.health_score;
            if ('performance_score' in result)
                metric.performance_score = result.performance_score;
            return metric;
     ...
     */
    mapBaselineStatusToHealth(status) {
        switch (status) {
            case 'NORMAL':
                return 'HEALTHY';
            case 'TRAFFIC_DROP':
    ...
    async batchProcessResults(results) {
        const processPromises = results.map(({ result, templateId, timeWindow }) => this.processQueryResult(result, templateId, timeWindow));
        await Promise.all(processPromises);
    }
    /**
     * Create a metrics processor that automatically stores results
     */
    createAutoProcessor(templateId, timeWindow) {
        return async (result) => {
