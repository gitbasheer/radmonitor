...
import { RadixTrie, SearchResult } from '../trie/radix-trie.js';
import { 
  EIDMetadata, 
  EIDHierarchy, 
  RADSet, 
...
export class EIDRegistry {
  ...
  private hotCache: Map<string, HotEIDEntry>;
  private hierarchyCache: Map<string, EIDHierarchy>;
  private recentEIDs: string[];
  private maxRecentItems: number = 20;
  private maxHotItems: number = 100;

  constructor() {
    this.trie = new RadixTrie<EIDMetadata>();
   ...
   */
  async initialize(historicalData?: EIDMetadata[]): Promise<void> {
    if (historicalData) {
   ...
   */
  getHotEIDs(limit: number = 10): HotEIDEntry[] {
   ...
   * Get recent EIDs
   */
  getRecentEIDs(limit: number = 10): EIDMetadata[] {
    return this.recentEIDs
      .slice(0, limit)
      .map(eid => this.trie.search(eid))
      .filter((metadata): metadata is EIDMetadata => metadata !== undefined);
  }
   ...
   */
  private updateHotScore(metadata: EIDMetadata): void {
    const now = Date.now();
    ...
    else if (ageInHours < 24) score *= 1.5;
    else if (ageInHours > 168) score *= 0.5; // Older than a week
    ...
    if (existing) {
      if (score > existing.score * 1.1) trend = 'rising';
      else if (score < existing.score * 0.9) trend = 'falling';
    }
   ...
   */
  private createSuggestion(result: SearchResult<EIDMetadata>, query: string): EIDSuggestion {
    const metadata = result.value;
   ...
   */
  private findMatchedPart(eid: string, query: string): string {
   ...
   * Update hierarchy cache
   */
  private updateHierarchy(metadata: EIDMetadata): void {
   ...
   * Track recent EIDs
   */
  private trackRecent(eid: string): void {
  ...
  private pruneHotCache(): void {
    if (this.hotCache.size <= this.maxHotItems) return;
...
// Keep only top N
  ...
  exportState(): {
    ...
    const hotScores = Array.from(this.hotCache.entries())
      .map(([eid, entry]) => [eid, entry.score] as [string, number]);
