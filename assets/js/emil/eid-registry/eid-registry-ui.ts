...
export class EIDRegistryUI {
  private options: EIDRegistryUIOptions;
  private currentEIDs: EIDWithMetrics[] = [];
  private selectedEIDs: Set<string> = new Set();
  private currentFilters: EIDSearchFilters = {};
  ...
  private attachEventListeners(): void {
    // Search input
    const searchInput = document.getElementById('eid-search') as HTMLInputElement;
    ...
    // Filter selects
    const typeFilter = document.getElementById('eid-filter-type') as HTMLSelectElement;
    typeFilter?.addEventListener('change', () => {
      this.currentFilters.type = typeFilter.value ? parseInt(typeFilter.value) : undefined;
      this.loadEIDs();
    });

    const statusFilter = document.getElementById('eid-filter-status') as HTMLSelectElement;
    statusFilter?.addEventListener('change', () => {
      this.currentFilters.status = statusFilter.value as EIDStatus || undefined;
      this.loadEIDs();
    });

    const healthFilter = document.getElementById('eid-filter-health') as HTMLSelectElement;
    ...
    // Registration form
    const form = document.getElementById('eid-registration-form') as HTMLFormElement;
   ...
   */
  private async loadEIDs(): Promise<void> {
  ...
  private renderEIDs(): void {
    ...
    grid.querySelectorAll('.eid-card').forEach(card => {
      card.addEventListener('click', (e) => {
        const eidId = (e.currentTarget as HTMLElement).dataset.eidId;
   ...
   */
  private renderEIDCard(eid: EIDWithMetrics): string {
   ...
   * Select an EID
   */
  private selectEID(eid: EIDWithMetrics): void {
  ...
  private async handleRegistration(): Promise<void> {
    ...
    const request: EIDRegistrationRequest = {
      id: (document.getElementById('eid-id') as HTMLInputElement).value,
      name: (document.getElementById('eid-name') as HTMLInputElement).value,
      is_rad: (document.getElementById('eid-is-rad') as HTMLInputElement).checked,
      is_experiment: (document.getElementById('eid-is-experiment') as HTMLInputElement).checked,
      status: (document.getElementById('eid-status') as HTMLSelectElement).value as EIDStatus,
      description: (document.getElementById('eid-description') as HTMLTextAreaElement).value || undefined,
      owner: (document.getElementById('eid-owner') as HTMLInputElement).value || undefined,
      tags: (document.getElementById('eid-tags') as HTMLInputElement).value
        .split(',')
        .map(t => t.trim())
   ...
   * Export EIDs to CSV
   */
  private async exportEIDs(): Promise<void> {
    try {
