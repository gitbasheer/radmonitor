...
export class EIDParser {
    static DEFAULT_NAMESPACE = 'pandc';
    static DEFAULT_RADSET = 'vnext';
    /**
     ...
     */
    static extractRADType(eid) {
        try {
            const parsed = this.parse(eid);
            const radId = parsed.radId || '';
            // Common RAD types based on existing patterns
            if (radId.includes('recommendation'))
                return 'recommendations';
            if (radId.includes('discovery'))
                return 'discovery';
            if (radId.includes('search'))
                return 'search';
            if (radId.includes('feed'))
                return 'feed';
            if (radId.includes('cart'))
                return 'cart';
            if (radId.includes('checkout'))
                return 'checkout';
     ...
     */
    static extractRADIdentifier(eid) {
        try {
            const parsed = this.parse(eid);
     ...
     */
    static getRADDisplayName(radIdentifier) {
        const radNames = {
     ...
     */
    static getHierarchyPath(eid) {
        try {
            const parsed = this.parse(eid);
            const path = [];
            if (parsed.namespace)
                path.push(parsed.namespace);
            if (parsed.radset)
                path.push(parsed.radset);
            if (parsed.radId)
                path.push(parsed.radId);
            if (parsed.subaction)
                path.push(parsed.subaction);
            if (parsed.action)
                path.push(parsed.action);
            return path;
        }
        catch {
            return [eid];
     ...
     */
    static createMetadata(eid, additionalData) {
        const parsed = this.parse(eid);
        return {
            eid,
...
//# sourceMappingURL=eid-parser.js.map